...
type
  ...
  public
    ...
    procedure   EnableTLS(const ACAFile: string = ''; const ACertFile: string = ''; const AKeyFile: string = ''; const VerifyPeer: Boolean = False);
  ...
  private
    ...
    // non standard
    procedure RxBrokerConnect(Sender: TObject; aProtocol: UTF8String; aVersion: byte; aClientID, aUserName, aPassword: UTF8String; aKeepAlive: Word; aClean: Boolean);
    procedure SetMaxRetries(const Value: integer);
    procedure SetRetryTime(const Value: cardinal);
  public
...
begin
  ...
  for i := 1 to length (aTopic) do if aTopic[i] = '/' then Result.Add('') else Result[Result.Count - 1] := Result[Result.Count - 1] + Char (aTopic[i]);
end;
...
begin
  ...
  if not MultiLevel then Result := (s.Count = t.Count) else Result := (s.Count <= t.Count + 1);
  if Result then
    begin
      for i := 0 to s.Count - 1 do
        begin
          ...
          else Result := Result and (s[i] = t[i]);
...
begin
  if State = wsConnected then
    begin
      ...
      if (TMQTTQOSType ((x and $06) shr 1) in [qtAT_LEAST_ONCE, qtEXACTLY_ONCE]) and (TMQTTMessageType ((x and $f0) shr 4) in [{mtPUBREL,} mtPUBLISH, mtSUBSCRIBE, mtUNSUBSCRIBE]) and (anID > 0) then begin
...
procedure TClient.DoSetWill(Sender: TObject; aTopic, aMessage: UTF8String; aQos: TMQTTQOSType; aRetain: boolean);
begin
...
begin
  ...
  if Owner is TWSocketServer then
    if Owner.Owner is TMQTTServer then aServer := TMQTTServer (Owner.Owner);
...
begin
  ...
  if changed and  Assigned (FOnSubscriptionChange) then FOnSubscriptionChange(Self);
...
begin
  ...
  else mon ('Received Message from a Broker - Publishing..');
...
begin
  ...
  for i := 0 to Brokers.Count - 1 do SyncBrokerSubscriptions (TMQTTClient (Brokers[i]));
end;
...
procedure TMQTTServer.RxPublish (Sender: TObject; anID: Word; aTopic : UTF8String; aMessage: AnsiString);
var
  aParser : TMQTTParser;
  aClient : TClient;
  aMsg    : TMQTTMessage;
...
begin
  ...
  case aParser.RxQos of
    qtAT_MOST_ONCE  : PublishToAll (aClient, aTopic, aMessage, aParser.RxQos, aParser.RxRetain);
    qtAT_LEAST_ONCE :
      begin
    ...
    qtEXACTLY_ONCE  :
      begin
        ...
        else mon (string (aClient.Parser.ClientID) + ' Message ' + IntToStr (anID) + ' already stored.');
...
begin
  ...
  for i := 0 to Server.ClientCount - 1 do TClient(Server.Client[i]).Parser.MaxRetries := Value;
  for i := 0 to Brokers.Count - 1 do      TMQTTClient(Brokers[i]).Parser.MaxRetries   := Value;
end;
...
begin
  ...
  for i := 0 to Server.ClientCount - 1 do TClient(Server.Client[i]).Parser.KeepAlive := Value;
  for i := 0 to Brokers.Count - 1      do TMQTTClient(Brokers[i]).Parser.KeepAlive   := Value;
end;
...
begin
  ...
  with TIniFile.Create (anIniFile) do
    begin
      ReadSections (Sections);
      for i := 0 to Sections.Count - 1 do if Copy (Sections[i], 1, 6) = 'BROKER' then EraseSection (Sections[i]);
      for i := 0 to Brokers.Count - 1 do
        begin
          aBroker := Brokers[i];
        ...
        begin
          if IsSubscribed (UTF8String (aClient.Subscriptions[j]), aTopic) then
            begin
              ...
              if aClient.FBroker then mon ('Publishing to Broker ' + string (aClient.Parser.ClientID) + ' "' + string (aTopic) + '" Retained ' + ny[wasRetained and aClient.FBroker])
              else mon ('Publishing to Client ' + string (aClient.Parser.ClientID) + ' "' + string (aTopic) + '"');
                              ...
                              if bPacket.Counter = 0 then
                                begin
                                  ...
                                  if bPacket.Retries <= aClient.Parser.MaxRetries then
                                    ...
                                    begin
                                      WillClose := true;
                                      if Assigned (FOnFailure) then FOnFailure (aClient, frMAXRETRIES, WillClose);
                                      if WillClose then aClient.CloseDelayed;
                                    end;
                                end;
                            end;
...
procedure TMQTTServer.DoClientConnect(Sender: TObject; Client: TWSocketClient; Error: Word);
begin
  if Sender = Server then Mon('Client Connected...');
end;
...
procedure TMQTTServer.DoClientDisconnect (Sender: TObject; Client: TWSocketClient; Error: Word);
var
    ...
    begin
      ...
      if not FGraceful then
        begin
          ...
          if Assigned (FOnObituary) then FOnObituary (Client, aTopic, aMessage, aQos);

          PublishToAll (nil, aTopic, AnsiString (aMessage), aQos);
        end;
    end;

  if Assigned (FOnClientsChange) then FOnClientsChange (Server, Server.ClientCount - 1);
end;
...
procedure TMQTTServer.RxBrokerConnect(Sender: TObject; aProtocol: UTF8String; aVersion: byte; aClientID, aUserName, aPassword: UTF8String; aKeepAlive: Word; aClean: Boolean);
var
  aClient : TClient;
begin
  if not (Sender is TMQTTParser) then exit;
...
begin
  ...
  if Assigned (FOnCheckUser) then FOnCheckUser (aServer, aUserName, aPassword, Allowed);
  if Allowed then
    begin
      if aVersion < MinVersion then
        ...
        begin
          ...
          if not aClean then if Assigned (FOnRestoreSession) then FOnRestoreSession (aClient, aClientID) else Sessions.RestoreSession (aClientID, aClient);
          if Assigned (FOnDeleteSession) then FOnDeleteSession (aClient, aClientID) else Sessions.DeleteSession (aClientID);

          aClient.Parser.SendConnAck (rcACCEPTED);
          aClient.FGraceful := false;

          mon ('Accepted. Is Broker ' + ny[aClient.FBroker]);
          if Assigned (FOnClientsChange) then FOnClientsChange (aServer, aServer.ClientCount);
        end;
    end
  else
    begin
...
procedure TMQTTServer.RxHeader (Sender: TObject; MsgType: TMQTTMessageType; Dup: Boolean; Qos: TMQTTQOSType; Retain: Boolean);
begin
...
begin
  ...
  if Enable then SetTimer (Timers, 1, 100, nil) else
    begin
      KillTimer (Timers, 1);
      KillTimer (Timers, 2);
      KillTimer (Timers, 3);
        ...
        begin
          for i := 0 to Subscriptions.Count - 1 do
            begin
              ...
              Subscriptions.Objects[i] := TObject (x + (anID shl 16));
              Parser.SendSubscribe (anID, UTF8String (Subscriptions[i]), TMQTTQOSType (x));
            end;
        end;
    end
  else Activate (false); // not going to connect
end;

// publishing
procedure TMQTTClient.RxPublish (Sender: TObject; anID: Word; aTopic : UTF8String; aMessage : AnsiString);
var
  aMsg : TMQTTMessage;
begin
  case Parser.RxQos of
    qtAT_MOST_ONCE  : if Assigned (FOnMsg) then FOnMsg (Self, aTopic, aMessage, Parser.RxQos, Parser.RxRetain);
    qtAT_LEAST_ONCE :
      begin
    ...
    qtEXACTLY_ONCE  :
      begin
        ...
        else mon ('Message ' + IntToStr (anID) + ' already stored.');
      end;
  end;
end;
...
begin
  ...
  else Mon ('REC Message ' + IntToStr (anID) + ' not found.');
  Parser.SendPubRel (anID);
end;
...
function TMQTTClient.StartsWithPemHeader(const Path, Header: AnsiString): Boolean;
var
  FS  : TFileStream;
  Buf : AnsiString;
  L   : Integer;
begin
  ...
  try
    ...
    if L > 0 then begin
      ...
      // Remove poss�vel BOM UTF-8
      if (Length(Buf) >= 3) and (Buf[1] = #$EF) and (Buf[2] = #$BB) and (Buf[3] = #$BF) then Buf := Copy(Buf, 4, MaxInt);
...
begin
  ...
  for i := 0 to Topics.Count - 1 do
    begin
      ...
      if not found then Subscriptions.AddObject (Topics[i], TObject (x));
    end;
...
begin
  ...
  if not found then Subscriptions.AddObject (string (aTopic), TObject (x));
...
begin
  if Link.State = wsConnected then
    begin
      ...
      if (TMQTTQOSType ((x and $06) shr 1) in [qtAT_LEAST_ONCE, qtEXACTLY_ONCE]) and (TMQTTMessageType ((x and $f0) shr 4) in [{mtPUBREL,} mtPUBLISH, mtSUBSCRIBE, mtUNSUBSCRIBE]) and (anID > 0) then
        begin
...
procedure TMQTTClient.EnableTLS(const ACAFile, ACertFile, AKeyFile: string; const VerifyPeer: Boolean);
...
begin
  ...
  with Link.SslContext do begin
    ...
    if Cert <> '' then begin
      if not StartsWithPemHeader(Cert, '-----BEGIN CERTIFICATE-----') then raise ESslContextException.Create('Arquivo de certificado inv�lido ou n�o-PEM: ' + Cert);
      SslCertFile := Cert;
    end;

    if Key <> '' then begin
      if not (StartsWithPemHeader(Key, '-----BEGIN PRIVATE KEY-----') or StartsWithPemHeader(Key, '-----BEGIN RSA PRIVATE KEY-----')) then raise ESslContextException.Create('Arquivo de chave inv�lido ou n�o-PEM: ' + Key);
      SslPrivKeyFile := Key;
    end;
  end;
    ...
    begin
      ...
      for j := 0 to Subscriptions.Count - 1 do
        begin
          ...
          if (hiword (x) = anID) and ((x and $0000ff00) shr 8 = i) then Subscriptions.Objects[j] :=  TObject ($ff00 + ord (Qoss[i]));
        end;
    end;
end;
...
begin
  ...
  if ErrCode = 0 then begin
      ...
      if aClientID = ''         then aClientID := 'CID' + UTF8String (Link.GetXPort); // + TimeString;
      ...
      if FBroker then Parser.SendBrokerConnect (aClientID, Parser.UserName, Parser.Password, KeepAlive, Parser.Clean) else Parser.SendConnect (aClientID, Parser.UserName, Parser.Password, KeepAlive, Parser.Clean);
  end else begin
...
var
  i : integer;
begin
  for i := 0 to List.Count - 1 do TMQTTPacket (List[i]).Free;
  List.Clear;
end;
...
begin
  if (Index >= 0) and (Index < Count) then Result := List[Index] else Result := nil;
end;
...
begin
  if (Index >= 0) and (Index < Count) then List[Index] := Value;
end;
...
{ TMQTTMessageStore }
function TMQTTMessageStore.AddMsg (anID: Word; aTopic : UTF8String; aMessage : AnsiString; aQos : TMQTTQOSType; aRetry, aCount: cardinal; aRetained : Boolean) : TMQTTMessage;
begin
...
var
  i : integer;
begin
  for i := 0 to List.Count - 1 do TMQTTMessage (List[i]).Free;
  List.Clear;
end;
...
var
  i : integer;
begin
  for i := 0 to List.Count - 1 do TMQTTSession (List[i]).Free;
  List.Clear;
end;
...
procedure TMQTTSessionStore.RestoreSession (ClientID: UTF8String; aClient: TClient);
var
  aSession : TMQTTSession;
begin
...
procedure TMQTTSessionStore.StoreSession (ClientID: UTF8String; aClient: TClient);
var
  aSession : TMQTTSession;
begin
  aSession := GetSession (ClientID);
